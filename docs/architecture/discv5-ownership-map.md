...
The same Zig 0.16.0 ReleaseSafe executable and options measured the retry refactor against parent `0ac82b87`. Stack-probe allocations fell from 47,272 to 37,272 bytes for `flow.maintenance.run` and from 21,656 to 14,504 bytes for `Actor.applyEffectCompletion`; `RuntimeImpl.handleCommand` changed from 27,168 to 27,136 bytes. The surviving out-of-line `prepareFreshRetry` frame is 200 bytes; retained preparation and retry completion inline without standalone symbols. The parent retry paths copied the 7,760-byte response and 2,616-byte phase during preparation and reconstructed the 11,768-byte rollback owner during completion. The final paths contain no real `memcpy` or `memmove` call with `ActiveRequest` (11,840), `StoredRequest` (11,848), response, or phase length; only the compact 1,336-byte retry transition may move locally.
