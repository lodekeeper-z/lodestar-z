...
- During the initial staged slice, direct/internal Actor callers remained behavior-compatible through a temporary synchronous adapter. The completed architecture below removed that adapter path by path.
...
- In that staged slice, `sendTracked` was removed and internal health, eviction, and queue-drain paths temporarily used the same preparation/completion contract through a synchronous compatibility executor. The completed architecture below removed that executor.
...
- `retry`: exact `RetryHandle` (`RequestHandle` plus non-wrapping retry-send generation) and packet bytes only. The canonical `ActiveRequest` remains at a stable address and owns an optional compact `retry_send` substate containing only the retry-send generation, next deadline, and retained/fresh preparation. The active request continues to own its request generation, origin, response accumulator, phase, current permit, attempts, queued intent, and handshake state in place. A fresh preparation additionally owns only its bounded future transition and moved replacement permit until exact completion;
...
| DiscV5 tests after migration | 307 | 434 after request/response/retry/challenge ledger canonicalization, exact admission correlation, initial-response and response-source-handshake generation preflights, compact in-place handshake and retry rollback, and runtime/auth race tracers |
...
- **Retry completion:** only the exact request generation and retry-send generation may clear the compact substate. The canonical request remains `.active` at the same address throughout preparation and completion, while timeout, ordinary response/RPC access, challenge/handshake work, pending-candidate promotion, second retry, and lane redrain are gated. Retained completion consumes one attempt/deadline without moving the response, phase, or current permit. Fresh `.sent` installs the prepared phase and nonce, resets multipart state, swaps the exact permit, and consumes one attempt/deadline. Fresh `.failed` or `.runtime_stopped` releases only the prepared permit, preserves the prior phase and current permit, and consumes the same policy. Explicit cancellation, shutdown, and deinit release the current and optional prepared permits exactly once. Duplicate, stale, canceled, timed-out, key-reused, or post-shutdown completions are no-ops.
...
- **Response-source handshake publication:** exact preflight checks the endpoint, nonce, response generation, complete recoverable owner, and checked handshake successor before expected-credit commit, recovery copying, ephemeral key generation, signing, randomness, encoding, or effect publication. Failure removes that exact recovery and releases only its permit while leaving an incoming competing expected-credit receipt armed for Runtime rollback. `beginHandshake` repeats the complete exact-owner and checked-successor validation at the canonical mutation boundary.
...
Pending request and response candidate generations are revalidated after decrypt/decode and test replacement hooks, before exact credit commit. HANDSHAKE revalidates the captured challenge after signature, decrypt, and decode, then commits before challenge removal, session/peer mutation, event publication, or RPC dispatch. Request-source WHOAREYOU commits after exact source preflight and before cryptographic work. Response-source WHOAREYOU also preflights first, but its failure path consumes the exact stale recovery without committing the incoming expected credit; successful preflight commits before recovery copying or cryptographic work.
...
| `RetrySendState` | 1,336 |
| `ActiveRequest` | 11,840 |
| `StoredRequest` | 11,848 |
...
Compact effects are forbidden from regaining admission/permit, challenge/preparation source, keys, deadline, plaintext, destination, ENR, or transition ownership. Admission correlation remains canonical in the permit/credit ledger and did not enlarge `ActorEffect`. The canonical challenge backing is checked as `C + 1`; configuration rejects `C = 0`, and checked addition rejects overflow before allocation. Request storage remains bounded by `max_active_requests`; compact retry rollback adds no map, cache, allocation, or indirection.

The Zig 0.16.0 ReleaseSafe measurement for the compact rollback refactor against parent `0ac82b87` found stack-probe allocations of 37,272 bytes for `flow.maintenance.run`, down from 47,272, and 14,504 bytes for `Actor.applyEffectCompletion`, down from 21,656; `RuntimeImpl.handleCommand` changed from 27,168 to 27,136 bytes. The surviving out-of-line `prepareFreshRetry` frame is 200 bytes; retained preparation and retry completion inline without standalone symbols. The parent retry paths copied the 7,760-byte response and 2,616-byte phase during preparation and reconstructed the 11,768-byte rollback owner during completion.

A follow-up measurement used the same ReleaseSafe executable and disassembly method against `136cd612`. Borrowing the canonical phase during timeout retry reduced the `flow.maintenance.run` stack-probe allocation from 37,272 to 33,800 bytes and removed its residual 2,610-byte phase-payload `memcpy`. `challengeForPhase` now accepts a phase pointer, all callers pass pointers, and its production symbol contains no `memcpy` or `memmove`. Final retry paths contain no full `ActiveRequest` (11,840), `StoredRequest` (11,848), `Response` (7,760), or `Phase` (2,616) copies. Only compact state such as the 1,336-byte retry transition may move locally.
